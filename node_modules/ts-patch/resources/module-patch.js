        ...
        function createTransformerFromPattern(opt) {
            ...
            switch (config.type) {
                case "ls":
                    ...
                    transformerFn = factory(ls, cleanConfig);
                    break;
                case "config":
                    transformerFn = factory(cleanConfig);
                    break;
                case "compilerOptions":
                    transformerFn = factory(program.getCompilerOptions(), cleanConfig);
                    break;
                case "checker":
                    transformerFn = factory(program.getTypeChecker(), cleanConfig);
                    break;
                case undefined:
                case "program":
                    const { addDiagnostic, removeDiagnostic, diagnostics } = tsp.diagnosticExtrasFactory(program);
                    transformerFn = factory(program, cleanConfig, {
                        ts: ts,
                        addDiagnostic,
                        removeDiagnostic,
                        diagnostics,
                    ...
                    break;
                case "raw":
                    transformerFn = (ctx) => factory(ctx, program, cleanConfig);
                    break;
                default:
            ...
            const res = after ? ({ after: wrapper }) :
                afterDeclarations ? ({ afterDeclarations: wrapper }) :
                    { before: wrapper };
            return res;
        }
